...
if ! docker compose exec api php artisan db:show > /dev/null 2>&1; then
...
docker compose exec api php artisan db:show --counts
...
print_info "You can now access the application at: https://mediahub.argoflux.com"